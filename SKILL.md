...
description: 人文社科论文写作全流程指导。适用于文学、历史、哲学、社会学、文化研究等领域的中文学术论文。当用户提到"论文""写论文""选题""文献综述""论文修改""论文结构""摘要翻译""帮我查文献""参考文献格式""论文没有新意""理论和文本脱节""章节之间缺乏逻辑""帮我检查论文""摘要翻译成英文""投稿准备""脚注格式"等场景时触发。覆盖从选题到投稿的全流程，包含防幻觉规则、8个学术数据库聚合搜索、21条文本评估规则。
---

# 人文社科论文写作全流程指导

> **适用范围**：文学、历史、哲学、社会学、文化研究等人文社科领域的中文学术论文。
>
> **适配 Agent**：Claude Code / OpenClaw。本 skill 的所有指令、文件读取、脚本调用均基于这两个 agent 的能力设计。
...
2. **你的研究对象是什么？** 具体到作品名 / 历史事件 / 哲学文本。如果用户说"我想研究鲁迅"，追问：鲁迅的哪部作品或哪个时期？
...
详细维度说明见 `references/text-review.md`。

**评估系统的边界**：评估脚本只检查确定性规则（格式、用词、结构信号），不能判断论点说服力和论证逻辑连贯性。后者需要结合方法论指导做人工判断。搜索脚本中通过网页抓取的数据源（知网、万方、国家哲社文献中心）依赖网站结构，如果网站改版可能需要更新脚本。Google Scholar 的免费直接抓取模式不稳定，推荐配置 SerpAPI Key。
...
2. PDF 文件：先用 `python` 调用 `PyMuPDF`（`fitz`）或 `pdfplumber` 提取文本。如果是扫描件，尝试 OCR（`pytesseract`）
3. DOCX 文件：用 `python-docx` 读取
4. 如果提取失败，告知用户并建议将关键章节粘贴到对话中
5. 提取成功后，进入 `references/literature-analysis.md` 的拆解流程

**OpenClaw 环境**：
1. 检查 agent 是否提供了文件读取工具
2. 如果提供了，直接调用
3. 如果没有，引导用户将文献内容粘贴到对话中

**通用规则**（遵守 Rules R3）：
- 只分析提取到的原文内容，不推测未读到的部分
- 如果只提取到了部分内容（如OCR质量差），标注哪些部分是可靠的、哪些部分可能有误
...
| 文件 | 内容 | 加载时机 |
|------|------|---------|
| `references/writing-templates.md` | 引言、摘要、章节结尾、过渡段模板 | 用户进入写作阶段 |
| `references/theory-frameworks.md` | 16位理论家速查 + 框架搜索兜底 | 用户选择理论框架 |
| `references/terminology-bilingual.md` | 224条术语中英法德对照表 + 查询兜底 | 翻译或查术语英文 |
| `references/formatting-guide.md` | 字体字号、脚注、参考文献格式 | 用户问排版格式 |
| `references/literature-review.md` | 文献综述方法 + 搜索脚本说明 | 用户做文献综述 |
